...
public class LineConvolutionCalculator {
   ...
   */
  public BufferedImage calculate(Point2D.Double[][] tangentVectors, BufferedImage image, 
      int kernelLength, double roundoff) {
    // resultant image
    ...
    for (int i = 0; i < image.getHeight(); i++) {
      for (int j = 0; j < image.getWidth(); j++) {
        ...
        ArrayList<Point2D.Double> lineSegmentPoints = getLineSegmentPoints(tangentVectors, new Point2D.Double(j, i));
   ...
   */
  public ArrayList<Point2D.Double> getLineSegmentPoints(Point2D.Double[][] tangentVectorsField, Point2D.Double x) {
    ...
    double deltaS = shortestDistanceToEdge(currentVector, px, py);
    deltaS *= roundoff;
  ...
  /**
   * Calculate the shortest distance from point (x, y) to one of the four surrounding edges, along the vector direction.
   * 
   ...
   */
  protected double shortestDistanceToEdge(Point2D.Double vector, double x, double y) {
  ...
  /**
   * Gets the kernelLength.
   * 
   * @return the kernelLength
   */
  ...
  /**
   * Sets the kernelLength.
   * 
   * @param kernelLength the kernelLength to set
   */
